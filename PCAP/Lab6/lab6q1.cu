...
__global__ void convolution1D(int *input, int *mask, int *output, int width, int mask_width) {
...
int main() {
    ...
    cudaMemcpy(d_mask, h_mask, bytes_mask, cudaMemcpyHostToDevice);
    
    dim3 dimGrid((int)ceil((float)width / BLOCK_SIZE));
    dim3 dimBlock(BLOCK_SIZE);
    
    convolution1D<<<dimGrid, dimBlock>>>(d_input, d_mask, d_output, width, mask_width);
